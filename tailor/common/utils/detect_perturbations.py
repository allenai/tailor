...
from tailor.common.utils.head_prompt_utils import \
    get_vindex_by_tags, clean_prefix_for_one_tag, \
    gen_prompts_by_tags, convert_tag2readable, \
    gen_prompt_by_perturb_meta, is_equal_prompts, \
    parse_change_type_meta, parse_keyword_type, \
    get_keyword_candidates_for_span, parse_filled_prompt, get_unique_tags
...
def get_common_keywords_by_tag(data_path=DEFAULT_DATASET_PATH,
    ...
    if max_count is not None: raw_json = raw_json[:max_count]
...
def identify_tags_for_span(doc, predicted, start=None, end=None):
    ...
    possible_tags = []
    pred_dict = { 
        get_vindex_by_tags(v["tags"]): deepcopy(v["tags"]) for v in predicted }
    for vidx, raw_tags in pred_dict.items():
        ...
        for pred in local_preds:
            ...
            while local_end < len(clean_tags)-1 and clean_tags[local_end] == pred:
                local_end += 1
            span_type = "noncore"
            if pred =="ARG0" or pred=="ARG1": span_type = "core"
            elif pred == "V": span_type = "verb"
            possible_tags.append(Munch(
                raw_tags=raw_tags, vlemma=vlemma, span_raw_tag=pred,
    ...
    if not possible_tags:
        ...
        if root.i in pred_dict:
            possible_tags.append(Munch(
                raw_tags=pred_dict[root.i], vlemma=root.lemma_, span_raw_tag="O",
                start=start, end=end, span_type=None))
...
def detect_perturbations(
    ...
    frameset_path=DEFAULT_FRAME_SET_PATH, common_keywords_by_tag=None):
    candidates = identify_tags_for_span(doc, predicted, start, end)
    output = []
    for candidate in candidates:
        if candidate.span_type is None: continue
        ...
        if candidate.span_type == "core":
            perturb_strs = [
                ...
                (f"change_content", f"CORE({tag}:CHANGE_CONTENT)", "NOUN_CHUNKS,PREFIX,CONNECT,ROOT"),
            ...
            perturb_strs = [
                ...
                (f"change_content", f"NONCORE({tag}:CHANGE_CONTENT)", "NOUN_CHUNKS,PREFIX,CONNECT,ROOT"),
                (f"change_spec", f"NONCORE({tag}:CHANGE_SPECIFICITY(sparse))", "EXACT,UNCASED"),
        ...
        for name, perturb_str, keyword_str in perturb_strs:
            if candidate.start == 0 and "front" in name: continue
            if candidate.end == len(doc) and "back" in name: continue
            # if the span is already too long, skip
            if candidate.end - candidate.start > 5 and "spec" in name: continue
            ...
            orig_prompt = gen_prompts_by_tags(doc, None, candidate.raw_tags,
                return_prompt_type="concrete", nblanks=1,
                args_to_blank=args_to_blank, keyword_str=keyword_str,
                frameset_path=frameset_path)
            #core_idx = get_core_idxes_from_meta(case.meta)
            
            perturb_meta = parse_change_type_meta(perturb_str)
            perturbed = gen_prompt_by_perturb_meta(doc, candidate.raw_tags,
                perturb_meta, orig_prompt.meta, frameset_path=frameset_path,
