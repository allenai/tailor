...
def _normalize_chunk(doc, start_idx, end_idx):
    ...
    while start_idx < end_idx and (
        doc[start_idx].text in punctuation or doc[start_idx].is_punct or doc[start_idx].is_space
    ):
        start_idx += 1
    while start_idx < end_idx and (
        doc[end_idx - 1].text in punctuation
        or doc[end_idx - 1].is_punct
        or doc[end_idx - 1].is_space
    ):
        end_idx -= 1
...
def complete_tree_pair(aidx, bidx, adoc, bdoc, ops, adoc_range=None, bdoc_range=None):
    ...
    for full_tree in [False, True]:
        ...
        _bidx_candidate = list(bidx_extend) + [
            match_idx_in_eops(idx, ops, "from", True) for idx in aidx_extend
        ]
        bidx_new = [max(0, min(_bidx_candidate)), min(len(bdoc), max(_bidx_candidate))]
        _aidx_candidate = list(aidx_extend) + [
            match_idx_in_eops(idx, ops, "to", True) for idx in bidx_new
        ]
...
def compute_edit_ops(adoc, bdoc, only_return_edit=False):
    ...
    eops_raw = difflib.SequenceMatcher(
        a=[t.text for t in adoc], b=[t.text for t in bdoc]
    ).get_opcodes()
    eops = []
    for op, f1, f2, t1, t2 in eops_raw:
        if only_return_edit and (
            op == "equal"
            or (
                _normalize_chunk(adoc, f1, f2).lemma_.lower() in stopwords
                and _normalize_chunk(bdoc, t1, t2).lemma_.lower() in stopwords
            )
        ):
            continue
...
def load_generator(model_path="allenai/tailor"):
    ...
    # TODO: use cached_transformers instead.
