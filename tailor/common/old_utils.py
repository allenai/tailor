...
def identify_tags_for_span(doc, predicted, start=None, end=None):
    ...
    for vidx, raw_tags in pred_dict.items():
        ...
        local_preds = set([p for p in clean_tags[start:end] if p not in ["V", "O"]])
        vlemma = doc[vidx].lemma_
        ...
        for pred in local_preds:
            ...
            possible_tags.append(
                Munch(
                    ...
                    is_core=pred == "ARG0" or pred == "ARG1",
                )
            )
...
def fillin_prompt(prompt, generated_text, is_clean_prefix=False, return_sequence=True):
    """Fill in the prompt
    ...
    if not return_sequence:
        ...
        # fillins = re.split(r"<extra_id_[0-9]>", generated_text)
        #    if "<extra_id_0>" not in generated_text:
        #        generated_text = generated_text.replace("<pad>", "<extra_id_0>")
        # hack to insert space between two consecutive extra id tokens, since this means empty string but space may not be decoded
        generated_text = generated_text.replace("><extra_id", "> <extra_id")
        fillins = re.findall(r"<extra_id_(?P<id>[0-9]+)>(?P<fillin>[^<]+)", generated_text)
        prompt_fix = prompt
        # should have at least one fill in
        if all([fillin.strip() == "" for _, fillin in fillins]):
            raise BadGenerationError(f"No fillins in generated text: {generated_text}")
        #        return None
        for tid, fillin_ in fillins:
            # for tid, fillin_ in enumerate(fillins):
            fillin_ = fillin_.strip()
            # TODO: remove this EMPTY check with new generator trained to output "" for empty targets
            fillin = (
                fillin_
                if fillin_.lower() != "empty"
                and fillin_.lower() not in string.punctuation
                and difflib.SequenceMatcher(a="empty", b=fillin_.lower()).quick_ratio() <= 0.3
                else ""
            )
            if re.search(f"\[([A-Z0-9\-]+) \| <extra_id_{tid}>\]", prompt_fix):
                prompt_fix = prompt_fix.replace(f"<extra_id_{tid}>", fillin)
            else:
                prompt_fix = prompt_fix.replace(f"<extra_id_{tid}>", f"[FILL{tid} | {fillin}]")
        if re.search(r"<extra_id_[0-9]>", prompt_fix):
            raise BadGenerationError(
                f"Sentinel tokens remaining in generated text: {generated_text}"
            )
    #        return None
    else:
        header, _ = extract_header_from_prompt(prompt)
        ...
        for idx, exp in enumerate(
            ...
            if idx == 0:
                ...
                for m in re.finditer(exp, prompt):
                    ...
                    if all(
                        [(tag.end(0) <= m.start(0)) or (m.end(0) <= tag.start(0)) for tag in tags]
                    ):
                        tags.append(m)
    else:
        tags = list(re.finditer(r"\[(?P<tag>[^\|]+)(?P<punct> \| )(?P<fillin>[^\]]*)\]", prompt))
    vidx = -1
