...
def compute_phrase_perplexity(
    ...
    sentence_phrase_tuples = (
        sentence_phrase_tuples
        if type(sentence_phrase_tuples) != tuple
        else [sentence_phrase_tuples]
    )
...
def compute_delta_perplexity(edit_ops, perplex_scorer, is_normalize=False, is_cuda=True):
    ...
    for op in edit_ops:
        aphrase, bphrase = (
            ...
            if op.op == "insert" or op.op == "delete"
            else (op.fromz_core, op.toz_core)
        )
    ...
    paired_scores = sorted(
        paired_scores, key=lambda x: (max(x.pr_sent, x.pr_phrase)), reverse=True
    )  # use the most ungrammar part as the
