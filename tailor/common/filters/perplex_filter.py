...
def _tokens_log_prob(texts, model, tokenizer, batch_size=128, is_cuda=True):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        outputs.extend(_tokens_log_prob_for_batch(
            batch, model, tokenizer, is_cuda=is_cuda))
    return outputs
...
def _tokens_log_prob_for_batch(texts, model, tokenizer, is_cuda=True):
    ...
    with torch.no_grad():
        ...
        attention_mask = torch.from_numpy(np.asarray(
    ...
    for sent_index in range(len(texts)):
        ...
        sent_ids_scores = sent_logits.gather(
            1, sent_ids.unsqueeze(1)).squeeze(1)
        ...
        output = (sent_log_probs.cpu().numpy(),
                  sent_ids.cpu().numpy(), sent_tokens)
...
def load_perplex_scorer(model_id='gpt2', is_cuda=True):
    ...
    tokenizer = GPT2TokenizerFast.from_pretrained(
        model_id, use_fast=True, add_special_tokens=False)
    device = "cuda" if is_cuda else "cpu"
...
def compute_phrase_perplexity(
    ...
    scores = []
    sentence_phrase_tuples = sentence_phrase_tuples if type(
        sentence_phrase_tuples) != tuple else [sentence_phrase_tuples]
    if len(sentence_phrase_tuples) == 0:
        return scores
    model, tokenizer = perplex_scorer.model, perplex_scorer.tokenizer
    outputs = _tokens_log_prob(
        [s[0] for s in sentence_phrase_tuples], model, tokenizer, is_cuda=is_cuda)
    for idx, (sentence, phrase) in enumerate(sentence_phrase_tuples):
        log_probs_all = outputs[idx][0]
        full_len = len(outputs[idx][1]) - 1
        if phrase:
            prefix_len = len(
                tokenizer(sentence.split(phrase)[0].strip())["input_ids"])
        else:
            prefix_len = 0
        phrase_len = len(tokenizer(phrase)["input_ids"])
        prefix_idx, phrase_idx = [0, prefix_len], [
            prefix_len, prefix_len + phrase_len]
        ...
        full_sent_score = reduce_perplex_prob(
            log_probs_all, log=log, reduce=reduce)
...
def compute_delta_perplexity(edit_ops, perplex_scorer, is_normalize=False, is_cuda=True):
    ...
    # print(tuples)
    scores = compute_phrase_perplexity(
        tuples, perplex_scorer, is_normalize=is_normalize, is_cuda=is_cuda)
    # print(scores)
    paired_scores = []
