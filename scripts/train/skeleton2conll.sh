...
if [[ $(t $d) != "conll-formatted-ontonotes-5.0" ]]; then
  error "please make sure that you are pointing to the directory 'conll-formatted-ontonotes-5.0'"
fi
...
for language in arabic english chinese; do
  ...
  for partition in train development test conll-2012-test; do

    if [[ -d $d/data/$partition/data/$language/ ]]; then
      for skel in $(find $d/data/$partition/data/$language/ -name "*_skel"); do
