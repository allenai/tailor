...
from unlikelihood_seq2seq import Seq2SeqUnlikelihoodTrainer
...
@dataclass
class ULSeq2SeqTrainingArguments(Seq2SeqTrainingArguments):
    remove_unused_columns: bool = field(
        default=False,
        metadata={
            "help": "Whether to use unlikelihood training."
        },
    )
    ...
    use_resampling: bool = field(
        default=False,
        metadata={
            "help": "Whether to use temperature mixing during training."
        },
    )
    ul_alpha: float = field(
        default=1.0,
        metadata={
            "help": "mixing hyper-parameter for adding the penalty term, default to 100.0"
        },
    )
...
class DataTrainingArguments:
    ...
    reward_column: Optional[str] = field(
        default=None,
        metadata={"help": "The name of the column in the datasets containing annotation for positive/negative sentences. Used for unlikelihood training."},
    )
    weight_column: Optional[str] = field(
        default=None,
        metadata={"help": "The name of the column in the datasets containing sampling weights."},
    )
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, ULSeq2SeqTrainingArguments))
    ...
    # set the negative reward function
    reward_column = None
    if data_args.reward_column is not None:
        reward_column = data_args.reward_column
    else:
        assert (
            training_args.use_unlikelihood is False
        ), "Provide a reward column for using the reward unlikelihood"
        reward_column = dataset_columns[-1] if dataset_columns is not None else column_names[-1]

    # set weight for resampling
    weight_column = None
    if data_args.weight_column is not None:
        weight_column = data_args.weight_column
    else:
        assert (
            training_args.use_resampling is False
        ), "Provide a reward column for using the reward unlikelihood"
        weight_column = dataset_columns[-1] if dataset_columns is not None else column_names[-1]
    ...
    def preprocess_function(examples):
        ...
        if reward_column:
            model_inputs["rewards"] = examples[reward_column]
        if weight_column:
            model_inputs["weight"] = examples[weight_column]
        ...
        # TODO: check the format of model inputs
        return model_inputs
    ...
    trainer = Seq2SeqUnlikelihoodTrainer(
        model=model,
        args=training_args,
