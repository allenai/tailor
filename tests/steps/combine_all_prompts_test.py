...
class TestCombineAllPrompts(TailorTestCase):
    ...
    def test_combine_prompts(self):
        step = CombineAllPrompts()
        result = step.run([self.prompts, self.prompts])

        assert len(result) == len(self.processed_sentences)

    def test_length_mismatch_error(self):
        step = CombineAllPrompts()
        with pytest.raises(AssertionError):
            step.run([self.prompts, self.prompts[:1]])
